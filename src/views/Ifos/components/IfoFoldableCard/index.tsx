...
const Header = styled(CardHeader)<{ ifoId: string; $isCurrent?: boolean }>`
  ...
  background-image: ${({ ifoId }) => `url('/shib/images/ifos/${ifoId}-bg.svg'), url('/shib/images/ifos/${ifoId}-bg.png')`};
...
const NoHatBunny = ({ isLive, isCurrent }: { isLive?: boolean; isCurrent?: boolean }) => {
  ...
  return (
    <StyledNoHatBunny $isLive={isLive} $isCurrent={isCurrent}>
      <img
        src={`/shib/images/ifos/assets/bunnypop-${!isSmallerThanTablet ? 'right' : 'left'}.png`}
        width={123}
        height={162}
        alt="bunny"
      />
