...
const MIN_PRICE_USD_DISPLAYED = BigNumber.from(10)
const MIN_PRICE_BNB_DISPLAYED = BigNumber.from('1000000000000000')
const DISPLAYED_DECIMALS = 8
...
const formatPriceBnbDifference = ({
  price = BigNumber.from(0),
  minPriceDisplayed,
  unitPrefix,
  decimals,
  ...
  if (price.abs().lt(minPriceDisplayed)) {
    const signedPriceToFormat = minPriceDisplayed.mul(sign)
    return `<${unitPrefix}${formatBigNumberToFixed(signedPriceToFormat, 3, decimals)}`
  }

  return `${unitPrefix}${formatBigNumberToFixed(price, 3, decimals)}`
}
...
export const formatBnbv2 = (bnb: BigNumber) => {
  return formatPriceBnbDifference({ price: bnb, minPriceDisplayed: MIN_PRICE_BNB_DISPLAYED, unitPrefix: '', decimals: 18 })
}
